...
const SHELBYUSD_ASSET = '0x1b18363a9f1fe5e6ebf247daba5cc1c18052bb232efdc4c50f556053922d98e1'
...
export default function App() {
  ...
  // Fetch balances when wallet connected and on swap tab
  useEffect(() => {
    if (connected && walletAddr && tab === 'swap') {
      fetchBalances()
    }
  ...
  const fetchBalances = async () => {
    ...
    try {
      const addr = walletAddr || MY_ADDRESS
      const resp = await fetch(`${APTOS_API}/accounts/${addr}/resources`)
      if (resp.ok) {
        ...
        // ShelbyUSD balance
        const susdRes = resources.find(r => r.type?.includes(SHELBYUSD_ASSET))
        if (susdRes) setBalanceSUSD((parseInt(susdRes.data.coin?.value || susdRes.data.balance || 0) / 1e8).toFixed(4))
      }
    } catch (e) {
  ...
  const handleSwap = async () => {
    ...
    setSwapStatus('⏳ Swap in corso su Shelby testnet...'); setSwapStatusType('loading')
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${SHELBY_API}/blobs/${encodeURIComponent(Date.now()+'-'+file.name)}`, { method: 'PUT', headers, body: file })
      if (resp.ok) {
        const data = await resp.json().catch(() => ({}))
        blobId = data.blobId || data.blob_id || data.id; txHash = data.txHash || data.tx_hash; isReal = true
      }
    } catch (e) {}
    if (!blobId) {
    ...
    const entry = { name: file.name, blobId, txHash, size: file.size, wallet: walletAddr || null, date: new Date().toISOString(), isReal, explorerUrl: `${SHELBY_EXPLORER}/account/${walletAddr || MY_ADDRESS}` }
    const newHistory = [entry, ...history]
    setHistory(newHistory); localStorage.setItem('shelby_files', JSON.stringify(newHistory))
    setTimeout(() => {
      setUploading(false); setProgress(0)
      showStatus(`✅ Upload ${isReal ? 'REALE ✨' : 'simulato'} completato!\n📦 Blob: ${blobId}\n🔗 TX: ${txHash.slice(0,24)}...\n👛 ${walletAddr?.slice(0,10) || 'No wallet'}...`, 'success')
    }, 500)
  }
      ...
      {tab === 'upload' && (
        <>
          <div className="stats">
            ...
            <div className="stat"><div className="stat-value">Shelbynet</div><div className="stat-label">Network</div></div>
          </div>
          ...
          <div className="buttons">
            ...
            <button className="secondary" onClick={() => setShowHistory(h => !h)}>📋 {showHistory ? 'Nascondi' : 'I miei file'} ({history.length})</button>
              ...
              {history.length === 0
                ...
                : history.map((f, i) => (
                  <div className="file-item" key={i}>
                    <strong>#{history.length-i} 📄 {f.name}</strong><span className="gray"> ({(f.size/1024).toFixed(2)} KB)</span>
        ...
        <div className="swap-container">
          ...
          <div className="balance-box">
            <div className="balance-title">
              💰 Il tuo saldo Shelbynet
              <button className="btn-refresh" onClick={fetchBalances} disabled={loadingBalance}>
                {loadingBalance ? '⏳' : '🔄'}
              </button>
            </div>
            {!connected
              ...
              : (
                <div className="balance-grid">
                  <div className="balance-item">
                    <span className="balance-token">APT</span>
                    <span className="balance-amount">{balanceAPT ?? '—'}</span>
                  </div>
                  <div className="balance-item">
                    <span className="balance-token">ShelbyUSD</span>
                    <span className="balance-amount">{balanceSUSD ?? '—'}</span>
                  </div>
                </div>
              )
            }
          ...
          <div className="swap-box">
            ...
            <div className="swap-row">
              <div className="swap-token-box">
                <div className="swap-label">Da</div>
                <div className="swap-token">{swapFrom}</div>
              </div>
              <button className="btn-switch" onClick={() => setSwapFrom(t => t === 'APT' ? 'ShelbyUSD' : 'APT')}>⇄</button>
              <div className="swap-token-box">
                <div className="swap-label">A</div>
                <div className="swap-token">{swapTo}</div>
              </div>
            </div>
            <input
              className="swap-input"
              type="number"
              placeholder={`Importo in ${swapFrom}`}
              value={swapAmount}
          ...
          <div className="faucet-box">
            ...
            <div className="faucet-btns">
              <a href="https://faucet.shelbynet.shelby.xyz" target="_blank" rel="noreferrer" className="faucet-btn">
                💧 APT Faucet
              </a>
              <a href="https://faucet.shelbynet.shelby.xyz" target="_blank" rel="noreferrer" className="faucet-btn susd">
                💵 ShelbyUSD Faucet
              </a>
            </div>
            <p className="swap-note">oppure usa la CLI: <code>shelby faucet --network shelbynet</code></p>
          </div>
